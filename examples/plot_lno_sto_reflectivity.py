...
def make_lno_sto_superlattice(
    ...
    for _ in range(repeats):
        layers.append(
            layer_from_file(lno_file, energy_ev, thickness=lno_thickness, roughness=3.0)
        )
        layers.append(
            layer_from_file(sto_file, energy_ev, thickness=sto_thickness, roughness=3.0)
        )
    layers.append(layer_from_file(sto_file, energy_ev, thickness=0.0, roughness=3.0))
    return layers
...
def main() -> None:
    ...
    output_path = REPO_ROOT / "examples" / "lno_sto_reflectivity.png"
