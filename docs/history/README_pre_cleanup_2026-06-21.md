...
- Optional JAX nonlinear least squares with SciPy TRF
...
For the standalone JAX nonlinear least-squares optimizer:
...
- `src/swxps/jax_least_squares.py`: standalone JAX-Jacobian TRF least-squares optimizer.
...
## JAX Nonlinear Least Squares

The JAX least-squares backend is a third, standalone local optimizer. It uses
JIT-compiled residuals and JAX Jacobians with SciPy's bounded trust-region
reflective method; it does not replace either BO or L-BFGS-B. It is suited to
simultaneous reflectivity and multi-core-level SW-XPS rocking-curve fits when
the simulator callback has fixed JAX array shapes.

Reflectivity uses log-scale residuals by default. Each rocking-curve block is
normalized and weighted independently so point count and signal magnitude do
not dominate accidentally. Those choices should be checked for each
experimental dataset. Like L-BFGS-B, TRF is local and still needs a physically
