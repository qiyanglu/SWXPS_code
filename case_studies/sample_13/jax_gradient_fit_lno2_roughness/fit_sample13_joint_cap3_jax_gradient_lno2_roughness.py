"""Refine Sample#13 joint cap3 fitting with constrained LNO cap roughness.
...
LNO-1 is constrained to a very thin 1-20 A layer. Both the LNO-1 and LNO-2
upper-interface roughnesses are fitted through fractions constrained to stay
between 0 and the LNO-1 thickness. For rocking curves, LNO-1 contributes to
La 4d only; Ni 3p is emitted from LNO-2 only.
"""
...
PARAMETERS = (
    FitParameter("carbon_thickness", 2.0, 15.0, "Angstrom", initial=10.0),
    FitParameter("carbon_roughness_fraction", 0.0, 1.0, "", initial=0.25),
    FitParameter("top_lno_total_thickness", 45.0, 65.0, "Angstrom", initial=50.0),
    FitParameter("top_lno_layer1_thickness", 1.0, 20.0, "Angstrom", initial=3.0),
    FitParameter("lno1_roughness_fraction", 0.0, 1.0, "", initial=0.5),
    FitParameter("lno2_roughness_fraction", 0.0, 1.0, "", initial=0.5),
    FitParameter("sto_thickness_start", 13.0, 18.5, "Angstrom", initial=14.7803),
    FitParameter("lno_thickness_start", 13.0, 18.5, "Angstrom", initial=17.6706),
    FitParameter("sto_thickness_delta", 0.0, 3.0, "Angstrom", initial=0.0),
    FitParameter("lno_thickness_delta", 0.0, 3.0, "Angstrom", initial=0.509788),
    FitParameter("thickness_transition_repeat", 0.0, 39.0, "repeat", initial=39.0),
    FitParameter("thickness_transition_width", 1.0, 20.0, "repeat", initial=16.7076),
    ...
    FitParameter("reflectivity_angle_offset", -0.30, 0.30, "deg", initial=0.133279),
...
def main() -> None:
    ...
    print("Sample#13 joint cap3 JAX-gradient setup, constrained LNO roughness variant")
    run_dir = RUNS_ROOT / "jax_gradient_lno2_roughness" / args.output_dir
    ...
    print("Layer roughness: fitted LNO-1 and LNO-2 roughnesses, both <= LNO-1 thickness")
    print("Layer-selective RC emission: C 1s -> C; Ni 3p -> LNO-2; La 4d -> LNO-1 + LNO-2")
...
def lno1_roughness_from_values(values: dict[str, float]) -> float:
    """Return C/LNO-1 roughness constrained by LNO-1 thickness."""

    return values["lno1_roughness_fraction"] * values["top_lno_layer1_thickness"]
