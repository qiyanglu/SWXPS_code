"""Prepare and fit Sample#13 C/LNO/LNO-STO/STO experimental data with BO.
...
STAGED_REFLECTIVITY_BASELINE = {
    "top_lno_thickness": 163.318,
    "sto_thickness_start": 14.7803,
    "lno_thickness_start": 17.6706,
    "sto_thickness_delta": 0.0,
    "lno_thickness_delta": 0.509788,
    "thickness_transition_repeat": 39.0,
    "thickness_transition_width": 16.7076,
    "reflectivity_angle_offset": 0.133279,
}
...
def sample13_superlattice_template() -> SuperlatticeTemplate:
    """Return the corrected Sample#13 STO/LNO superlattice template."""
...
def build_reflectivity_stack(values: dict[str, float]):
    """Build the Sample#13 reflectivity stack from fit parameters."""
    ...
    parts = (
        ...
        *sample13_graded_superlattice_templates(values),
        LayerTemplate.from_file(
            "STO",
            STO_OPC_FILE,
            0.0,
...
def build_rc_stack(values: dict[str, float]):
    """Build the Sample#13 RC stack from fit parameters."""
...
def load_best_reflectivity_values(
    ...
    values = {**INITIAL_VALUES, **STAGED_REFLECTIVITY_BASELINE}
    if not path.exists():
        return values
...
def sample13_graded_superlattice_templates(values: dict[str, float]) -> tuple[LayerTemplate, ...]:
...
def make_fit_problem(
    ...
    """Create the Sample#13 fitting problem from prepared curves."""
...
def print_setup(
    ...
    print("Sample#13 proposed fitting setup")
...
def save_superlattice_profile_plot(values: dict[str, float], path: Path) -> None:
    ...
    fig.suptitle("Sample#13 Superlattice Profile")
    fig.tight_layout()
    fig.savefig(path, dpi=220)
    plt.close(fig)
...
def run_fit(
    ...
    output_prefix = {
        "joint": "sample13_bo",
        "reflectivity": "sample13_reflectivity_bo",
        "rc": "sample13_rc_bo",
    }[fit_mode]
    ...
    plot_stack_schematic(
        ...
        title=f"Sample#13 {fit_mode.title()} BO Stack",
        top_layers=5,
        bottom_layers=3,
    )
    if fit_mode == "reflectivity":
...
def _load_pyplot():
    try:
        ...
        raise ImportError("matplotlib is required for Sample#13 preview plots") from error
    return plt
...
def main() -> None:
    ...
    preview_path = CASE_DIR / "sample13_normalized_preview.png"
    ...
    stack_path = CASE_DIR / "sample13_initial_stack_schematic.png"
    plot_stack_schematic(
        stack_path,
        initial_stack,
        title="Sample#13 Initial Stack",
        top_layers=5,
        bottom_layers=3,
    )
