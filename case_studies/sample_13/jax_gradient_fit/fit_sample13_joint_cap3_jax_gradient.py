"""Refine Sample#13 joint cap3 fitting with the JAX backend and L-BFGS-B.
...
LNO-1 is constrained to a very thin 2-20 A layer. For rocking curves, this
first LNO layer contributes to La 4d only; Ni 3p is emitted from LNO-2 only.
"""
...
DEFAULT_OUTPUT_PREFIX = "sample13_joint_cap3_jax_gradient"
DEFAULT_RUN_DIR = "clean_single_50iter"
...
PARAMETERS = (
    ...
    FitParameter("cap_roughness_fraction", 0.0, 1.0, "", initial=0.5),
...
def main() -> None:
    ...
    print("Sample#13 joint cap3 JAX-gradient setup")
    run_dir = RUNS_ROOT / "jax_gradient" / args.output_dir
...
def cap_roughness_from_values(values: dict[str, float]) -> float:
    ...
    max_roughness = min(5.0, layer1_thickness)
    if max_roughness < 1.0:
        raise ValueError("top LNO layer 1 thickness must be at least 1 Angstrom")
    return 1.0 + values["cap_roughness_fraction"] * (max_roughness - 1.0)
...
def save_summary(
    ...
    row = {
        ...
        "cap_roughness": cap_roughness_from_values(result.best_parameters),
...
def write_attempt_summary(path: Path, records: list[dict[str, object]]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for record in records:
            ...
            row["cap_roughness"] = cap_roughness_from_values(result.best_parameters)
...
def print_result(result, best_numpy, best_jax, elapsed: float) -> None:
    ...
    print(f"    cap_roughness: {cap_roughness_from_values(result.best_parameters):.6g} A")
    print(
        "    top_lno_layer2_thickness: "
